...
struct HgenScratch {
  std::vector<double> m_hg;
  std::vector<double> m2b_hg;
  std::vector<double> a_hg;
  std::vector<double> vrec_hg;
  std::vector<double> tau_hg;

  explicit HgenScratch(std::size_t cap = 32) {
    m_hg.reserve(cap);
    m2b_hg.reserve(cap);
    a_hg.reserve(cap);
    vrec_hg.reserve(cap);
...
void hgen(Functions& func,
          double m1, double a1, double m2, double a2, double k2, double vesc, string stype, double Zmet,// star variables
  ...
  auto& m_hg    = sc.m_hg;
  auto& m2b_hg  = sc.m2b_hg;
  auto& a_hg    = sc.a_hg;
  auto& vrec_hg = sc.vrec_hg;
  ...
  if(nmerg_tot < 1){
    double P = func.rnd();
    if(P < nmerg_tot){ //We have more than one merger, we likely end mergers here
      success = "yes";       
  ...
  // If I have >1 merger in the cluster I can have a hierarchical secondary BH
    ...
    while (nmerg_budget > 0 && tmax >= tau && nbhs[0] > 0) {
      ...
      // if(tau*tcc > time){
      //   cout << "DEBUG_HGEN: "
      //       //  << "growth interrupted because tau > time "
      //        << "ID=" << id
      //        << " nrecy =" << nrecy 
      //        << " gen2=" << gen2
      //        << " time=" << time
      ...
      //        << " stellar BHs=" << nbhs[1]
      //        << " 1st gen BHs=" << nbhs[2]
      //        << " nmerg_left=" << nmerg_budget
      //        << " nmerg_tot=" << nmerg_tot
      //        << " available BHs=" << nbhs.size()
      //        << endl;
      // }
    }

    auto t1 = Clock::now();
    tcerr_growth += std::chrono::duration<double, std::milli>(t1 - t0).count();

    // Debug: stampa la catena gerarchica
    // for (int g = 0; g < nbhs.size(); g++) {
    //   cout << "DEBUG_HGEN_CHAIN: "
    //        << "ID=" << id
    //        << " nrecy=" << nrecy
    //        << " nbhs[" << g << "]=" << nbhs[g]
    //        << " nmerg_left=" << nmerg_budget
    ...
    // Here we grow the hierarchical companion BH until we reach the maximum generation allowed or it is ejected
    bool retained = true;   // updated at each step from recoil

    while(gen2 <= nrecy && gen2 < (int)nbhs.size()-1 && retained){
      ...
      do{
        if (!use_mix)        // If in the main we are not having a mixing we chose from SSE catalogs
          func.singBHt_new(zams_cat, remn_cat, tdel_cat, kick_cat, single_bh, vesc); 
        else                // If in the main we are having a mixing we chose from mixed catalogs
      ...
      // (no if: absorbed into the while condition via 'retained')
      retained = (s[3] <= vesc);
    ...
    auto t3 = Clock::now();
    tcerr_chain += std::chrono::duration<double, std::milli>(t3 - t2).count();


    // Debug: stampa la catena gerarchica
    // for (int g = 0; g < m_hg.size(); g++) {
    //   cout << "DEBUG_HGEN_CHAIN: "
    //        << "ID=" << id
    //        << " nrecy=" << nrecy
    //        << " gen=" << g
  ...
  if(ejected == 0){
    ...
    for(int g = m_hg.size(); g > 1; g--){
      ...
      interaction_rate = func.inter_rate(
                                    ...
                                    trelax, t12capt, tbbhform, tcc);            // timescales 
    ...
    if(paired) {
      // cout << "ID: " << id << " nrecy: " << nrecy << " => Hierarchical companion BH of gen " << interacting_gen-1 << " paired (IR=" << IR_at_trigger << ")" << endl;
  ...
  auto t5 = Clock::now();
  tcerr_pairing += std::chrono::duration<double, std::milli>(t5 - t4).count();
...
int main(){
  ...
  for(int i=Niso+Nyc;i<Niso+Nyc+Ngc+Nnc;i++){
    ...
    // if(i < Niso+Nyc+5 || (i >= Niso+Nyc+Ngc && i < Niso+Nyc+Ngc+5))
    // cout << "DEBUG_Z: i=" << i << " type=" << (i<Niso+Nyc+Ngc?"GC":"NC")
    //      << " red_del=" << red_del << " logz=" << logz1 << " Z=" << Z[i] << endl;
  ...
  if(MonoZ == "yes" || (sfr_iso=="single" && sfr_clu=="single") || TagR == "seed"){
    cout<<"WARNING: mono Z mode enabled (MonoZ="<<MonoZ<<" sfr_iso="<<sfr_iso<<" sfr_clu="<<sfr_clu<<" TagR="<<TagR<<")"<<endl;
  ...
  // Debug
  // int cnt_below=0, cnt_above=0, cnt_nan=0;
  // for(int i=Niso;i<Nsrc;i++){
  //     if(Z[i] <= met[0]) cnt_below++;
  //     if(Z[i] >= met[nmetal-1]) cnt_above++;
  //     if(std::isnan(Z[i])) cnt_nan++;
  // }
  // cout << "DEBUG_CLAMP: dyn_total=" << Nsrc-Niso
  //     << " at_Zmin=" << cnt_below
  //     << " at_Zmax=" << cnt_above
  //     << " NaN=" << cnt_nan << endl;
  ...
  // Debug
  // cout << "DEBUG_BINS: Z0=" << Z0 << " Z1=" << Z1
  //    << " dZ=" << dZ << " numZ=" << numZ << endl;
  // for(int k=0;k<numZ;k++)
  //   cout << "  bin " << k << ": [" << pow(10., Zmin+k*dZ)
  //        << ", " << pow(10., Zmin+(k+1)*dZ) << "] met=" << met[k] << endl;
  // end debug
  ...
  */
  ...
  for(int k=0;k<numme;k++){
    ...
    for(int i=Niso; i<Nsrc; i++){
      ...
      // if(i % 100 == 0)
      //   cout << "DEBUG: source " << i << " Z=" << Z[i] 
      //    << " bin=[" << pow(10.,Z0) << ", " << pow(10.,Z1) 
      //    << "] met=" << met[k] 
      //    << " k=" << k << endl;
      

      if( (Z[i] < pow(10.,Z1) && Z[i] >= pow(10.,Z0)) || ((MonoZ == "yes" || (sfr_clu == "single" && sfr_iso=="single")) && Z[i] == met[k])) {
        // cout<<"      -- -- -- source "<<i<<" in metallicity bin "<<met[k]<<" "<<npar_runtime+1<<" / "<<Npar[k]<<endl;
        ...
        */
        ...
        //Let's define the merger budget for this cluster
        double eps =  func.GWeff(pcluster,Z[i]);

        double nmerg_tot = min(eps * mhalf, init_bhs/2.0); //max number of mergers allowed in this cluster
        double nmerg_budget = nmerg_tot;
      ...
      nsafe += 1;
        ...
        */
        ...
        do{
          ...
          // for(int k=0;k<numZ;k++){
          //   cout << "DEBUG - ID: " << i << "Npar[" << k << "]: " << Npar[k] << endl;
          // }
          ...
          // cout << "ID: "<< i << " nrecy: " <<nrecy << " m1: "<< mpri << " time: "<< time <<endl;            
        ...
        if(time < Hubble){
          ...
          out<<real_id<<" "<<Z[i]<<" "<<nrecy<<" "<<nhigen <<" "<<cluster<<" "<<REC<<" "<<mpri<<" "<<msec<<" "<<apri<<" "<<asec<<" "<<Mrem[i]<<" "<<Srem[i]<<" "<<Xrem[i]<<" "<<Krem[i]<<" "<<tfor[i]<<" "<<time<<" ";
                    out<<pow(10.,mint)<<" "<<pow(10.,rint)<<" "<<vthre<<" "<<label<<" "<<semi_ej<<" "<<semi_gw<<" "<< init_bhs<<" "<<nbhs[0]<<" "<< nbhs[1] << " " << nbhs[2] << " "<< nbhs[3] << " "<< nbhs[4] << " "<< nbhs[5] << " " << nbhs_6plus << " "<< nmerg_tot <<" "<<mhalf*mclcorr<<" "<<rhalf*rclcorr<<" "<<zmer<<" "<<zfor<<" "<<tsmbh<<" "<<zsmbh<<" "<<mzero<<" "<<ecc<<" "<<sma<<" "<<acrit<<" "<<tmer<<" "<<Cosa[i]<<" "<<Cosb[i]<<" "<<Cosg[i]<<endl; //Eccentricity added to output
